    ...
    function realEstate(data) {
        ...
        function highlightDatapoint(d) {
            d3.selectAll("li.datapoint")
                .style("font-weight", function(p) {return p == d ? 900 : 100});

            d3.selectAll("g.datapoint")
                .select("path").style("stroke-width", function(p) {return p == d ? "3px" : "1px"});
        ...
        function tabletView() {
            ...
            screenWidth = parseFloat(
                d3.select("svg").node().clientWidth || d3.select("svg").node().parentNode.clientWidth
                );
